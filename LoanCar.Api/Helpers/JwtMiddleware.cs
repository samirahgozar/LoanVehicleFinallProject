...
namespace LoanCar.Api.Helpers
{
    public class JwtMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, IAuthService userService)
        {
            ...
            if (context.Request.Path == "/api/auth/login" || context.Request.Path == "/api/auth/register")
            {
                var user = new User { Id = 1, FirstName = "", LastName = "", Username = "" };
